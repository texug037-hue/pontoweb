...
function calcSaldo(e1,s1,e2,s2,sp,modalidade){
  ...
  const extra=tot>=TOLERANCIA?tot:0;
  const falta=tot<=-TOLERANCIA?Math.abs(tot):0;
...
function TelaLogin({onLogin}){
  const [email,setEmail]=useState(()=>{try{return localStorage.getItem("pw_email")||"";}catch{return "";}});
  ...
  const handleLogin=async()=>{
    ...
    const uLocal=USUARIOS_LOCAL.find(u=>u.email===email&&u.senha===senha);
    if(uLocal){
      try{localStorage.setItem("pw_email",email);}catch{}
      onLogin({id:uLocal.perfil==="admin"?6:1,nome:uLocal.nome,email,
        empresa:uLocal.empresa,cargo:uLocal.cargo,
        modalidade:uLocal.modalidade,perfil:uLocal.perfil});
      return;
    }
      ...
      <div style={{width:"100%",maxWidth:380,background:"#fff",borderRadius:16,
        ...
        <div style={{padding:"24px"}}>
          ...
          <button onClick={()=>{
              ...
              setCarregando(true); setErro("");
              // Login local primeiro
              const uL=USUARIOS_LOCAL.find(u=>u.email===emailVal&&u.senha===senhaVal);
              if(uL){
                try{localStorage.setItem("pw_email",emailVal);}catch{}
                onLogin({id:uL.perfil==="admin"?6:1,nome:uL.nome,email:emailVal,
                  empresa:uL.empresa,cargo:uL.cargo,modalidade:uL.modalidade,perfil:uL.perfil});
                return;
              }
              if(modoLogin==="rhid"){
...
export default function App(){
  ...
  const bater=()=>{
    ...
    if(idxAberto!==-1){
      ...
      const realIdx=arr.findIndex(r=>r.data===todosRegs[idxAberto].data);
      if(realIdx!==-1)arr[realIdx]=dia;
