...
// Converte HH:MM em minutos, ajustando para turnos noturnos que cruzam meia-noite
// Se o horário for menor que o de entrada (ex: saída 05h < entrada 17h), soma 24h
function toMinNoturno(t, refEntrada){
  if(!t) return null;
  const [h,m]=t.split(":").map(Number);
  let min=h*60+m;
  if(refEntrada!=null && min < refEntrada) min+=1440; // cruza meia-noite
  return min;
}
...
function calcSaldo(e1,s1,e2,s2,sp,modalidade){
  ...
  const entMin=toMin(e1);

  // Ajuste meia-noite: qualquer horário menor que a entrada soma +24h
  const s1m = s1 ? toMinNoturno(s1, entMin) : null;
  const e2m = e2 ? toMinNoturno(e2, entMin) : null;
  const s2m = toMinNoturno(s2, entMin);
  const spMin = toMinNoturno(sp||m.sp, entMin); // saída prevista (05:00)

  // Tempo trabalhado líquido (descontando janta)
  // Período 1: entrada até saída pra janta
  // Período 2: volta da janta até saída final
  const periodo1 = s1m ? s1m - entMin : 0;
  const periodo2 = (e2m && s2m) ? s2m - e2m : (!e2m && s2m && s1m) ? s2m - s1m : (!s1m && s2m) ? s2m - entMin : 0;
  const trab = periodo1 + periodo2;

  // Extra/falta: diferença entre saída real e saída prevista (05:00)
  // + cada minuto que a janta foi menor que 60min também conta como extra
  const diffSaida = s2m - spMin;

  // Minutos "ganhos" na janta (janta menor que 1h = tempo extra trabalhado)
  const jantaReal = (s1m && e2m) ? e2m - s1m : 60;
  const jantaBonus = Math.max(0, 60 - jantaReal); // minutos a menos na janta = minutos extras

  // Minutos "ganhos" na entrada (entrou antes das 17h)
  const epMin = toMinNoturno(m.ep, null); // 17:00 = 1020
  const entradaBonus = Math.max(0, epMin - entMin); // entrou antes = minutos extras

  const totalExtra = diffSaida + jantaBonus + entradaBonus;

  const extra = totalExtra >= TOLERANCIA ? totalExtra : 0;
  const falta = totalExtra <= -TOLERANCIA ? Math.abs(totalExtra) : 0;
  return { trab, extra, falta };
}
...
// ══════════════════════════════════════════════════════
const SERVIDOR_URL = "https://late-paper-379b.texug037.workers.dev";
const RHID_CID = "81212";
// Login local de fallback enquanto API RHiD não está disponível
const USUARIOS_LOCAL = [
  {email:"t.e.x.u.g.o@hotmail.com", senha:"280614", nome:"FABRICIO FERREIRA", empresa:"ILUMI INDUSTRIA E COMERCIO LTDA", cargo:"AUXILIAR ADMINISTRATIVO", modalidade:"12x36-noturno"},
  {email:"admin@empresa.com", senha:"admin", nome:"ADMINISTRADOR RH", empresa:"ILUMI INDUSTRIA E COMERCIO LTDA", cargo:"GESTOR DE RH", modalidade:"8h-diurno", perfil:"admin"},
];
...
function TelaLogin({onLogin}){
  const [email,setEmail]=useState(()=>{ try{ return JSON.parse(localStorage.getItem("login_ultimo")||'""'); }catch{ return ""; } });
  ...
  const [emailsHistorico,setEmailsHistorico]=useState(()=>{ try{ return JSON.parse(localStorage.getItem("login_historico")||"[]"); }catch{ return []; } });
  const [mostrarHistorico,setMostrarHistorico]=useState(false);

  const salvarEmailHistorico=(em)=>{
    if(!em) return;
    const lista=[em,...emailsHistorico.filter(e=>e!==em)].slice(0,5);
    setEmailsHistorico(lista);
    try{ localStorage.setItem("login_historico",JSON.stringify(lista)); localStorage.setItem("login_ultimo",JSON.stringify(em)); }catch{}
  };
  const removerEmailHistorico=(em)=>{
    const lista=emailsHistorico.filter(e=>e!==em);
    setEmailsHistorico(lista);
    try{ localStorage.setItem("login_historico",JSON.stringify(lista)); }catch{}
  };
  ...
  const handleLogin=async()=>{
    ...
    // Tenta login local primeiro (funciona sempre offline)
    const userLocal = USUARIOS_LOCAL.find(u=>u.email===email&&u.senha===senha);
    if(userLocal){
      salvarEmailHistorico(email);
      onLogin({id:userLocal.perfil==="admin"?6:1, nome:userLocal.nome, email,
        empresa:userLocal.empresa, cargo:userLocal.cargo,
        modalidade:userLocal.modalidade, perfil:userLocal.perfil||"funcionario"});
      return;
    }

    if(modoLogin==="rhid"){
      // Tenta login via RHiD
      try{
        ...
        salvarEmailHistorico(email);
        onLogin({id:1, nome:data.nome||data.name||email, email,
          empresa:data.empresa||"ILUMI INDUSTRIA E COMERCIO LTDA",
          cargo:data.cargo||"AUXILIAR ADMINISTRATIVO",
    ...
    if(email==="admin@empresa.com"&&senha==="admin"){
      onLogin({id:6,nome:"ADMINISTRADOR RH",email,empresa:"ILUMI INDUSTRIA E COMERCIO LTDA",
        cargo:"Gestor de RH",modalidade:"12x36-noturno",perfil:"admin"});
    } else {
      setErro("E-mail ou senha incorretos.");
      setCarregando(false);
    }
          ...
          <div style={{marginBottom:14,position:"relative"}}>
            ...
            <div style={{border:"1.5px solid #DDE1E7",borderRadius:8,padding:"11px 14px",
              ...
              <input id="login-email" type="email" value={email}
                onChange={e=>{setEmail(e.target.value);setMostrarHistorico(true);}}
              ...
              {emailsHistorico.length>0&&(
                <button onClick={()=>setMostrarHistorico(p=>!p)}
                  style={{background:"none",border:"none",cursor:"pointer",padding:0,color:"#9AA0AB",fontSize:10}}>▼</button>
              )}
            </div>
            {/* Dropdown histórico */}
            {mostrarHistorico&&emailsHistorico.length>0&&(
              <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",
                border:"1.5px solid #DDE1E7",borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,0.1)",
                zIndex:100,overflow:"hidden",marginTop:2}}>
                <div style={{padding:"6px 12px",fontSize:10,color:"#9AA0AB",fontWeight:700,
                  textTransform:"uppercase",letterSpacing:.8,borderBottom:"1px solid #F0F0F0",background:"#FAFAFA"}}>
                  Acessos recentes
                </div>
                {emailsHistorico.filter(e=>!email||e.toLowerCase().includes(email.toLowerCase())).map(em=>(
                  <div key={em} style={{display:"flex",alignItems:"center",padding:"10px 14px",
                    gap:10,cursor:"pointer",borderBottom:"1px solid #F5F5F5"}}
                    onMouseDown={()=>{setEmail(em);setMostrarHistorico(false);}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9AA0AB" strokeWidth="1.8" strokeLinecap="round">
                      <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
                      <polyline points="22,6 12,13 2,6"/>
                    </svg>
                    <span style={{flex:1,fontSize:13,color:"#1A1A2E"}}>{em}</span>
                    <button onMouseDown={e=>{e.stopPropagation();removerEmailHistorico(em);}}
          ...
          <button onClick={()=>{
              ...
              if(modoLogin==="rhid"){
                ...
                if(emailVal==="admin@empresa.com"&&senhaVal==="admin"){
                  salvarEmailHistorico(emailVal);
                  onLogin({id:6,nome:"ADMINISTRADOR RH",email:emailVal,empresa:"ILUMI INDUSTRIA E COMERCIO LTDA",
                    cargo:"Gestor de RH",modalidade:"12x36-noturno",perfil:"admin"});
...
// ── Componente externo TelaRhid — fora do App para não perder foco nos inputs ──
function TelaRhidComp({rhidConectado,rhidSincMens,rhidCfg,rhidEmail,setRhidEmail,rhidSenha,setRhidSenha,onLogin,onDesconectar,onSincNow}){
  return(
    <>
      <div style={{background:rhidConectado?"#F0F7F2":"#FFF8F0",borderRadius:12,padding:"16px",
        ...
        {rhidConectado&&<div style={{width:10,height:10,borderRadius:"50%",background:VE,boxShadow:`0 0 0 3px ${VE}33`,flexShrink:0}}/>}
      </div>

      <div style={{background:BR,borderRadius:12,padding:"16px",marginBottom:12,border:`1px solid ${BD}`,boxShadow:SD}}>
        <div style={{fontWeight:600,fontSize:14,color:TX,marginBottom:12}}>Como funciona</div>
        {[
          ...
          <div key={i} style={{display:"flex",gap:12,marginBottom:i<2?12:0}}>
            <div style={{width:28,height:28,borderRadius:"50%",background:A,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:13,flexShrink:0}}>{s.n}</div>
            <div>
      ...
      {!rhidConectado?(
        <div style={{background:BR,borderRadius:12,padding:"16px",border:`1px solid ${BD}`,boxShadow:SD}}>
          <div style={{fontWeight:600,fontSize:14,color:TX,marginBottom:4}}>Conectar ao RHiD</div>
          <div style={{fontSize:12,color:TX2,marginBottom:16,lineHeight:1.5}}>
            Use o mesmo e-mail e senha que você usa em <span style={{color:A,fontWeight:600}}>rhid.com.br</span>
          </div>
          <div style={{border:`1px solid ${BD}`,borderRadius:8,padding:"10px 14px",background:BR,marginBottom:10}}>
            <div style={{fontSize:11,color:A,fontWeight:600,marginBottom:3,letterSpacing:.3}}>E-mail</div>
            <input type="email" value={rhidEmail} onChange={e=>setRhidEmail(e.target.value)}
              placeholder="seu@email.com"
              style={{width:"100%",border:"none",outline:"none",fontSize:14,color:TX,background:"transparent",fontFamily:"inherit"}}/>
          </div>
          <div style={{border:`1px solid ${BD}`,borderRadius:8,padding:"10px 14px",background:BR,marginBottom:16}}>
            <div style={{fontSize:11,color:A,fontWeight:600,marginBottom:3,letterSpacing:.3}}>Senha</div>
            <input type="password" value={rhidSenha} onChange={e=>setRhidSenha(e.target.value)}
              placeholder="••••••••"
              style={{width:"100%",border:"none",outline:"none",fontSize:14,color:TX,background:"transparent",fontFamily:"inherit"}}/>
          </div>
          <div style={{fontSize:11,color:TX2,background:"#F7F8FA",borderRadius:8,padding:"10px 12px",marginBottom:16,lineHeight:1.6}}>
            🔒 Suas credenciais ficam salvas apenas no seu dispositivo.
          </div>
          <button onClick={onLogin}
            style={{width:"100%",padding:"14px",borderRadius:8,border:"none",background:A,color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer",fontFamily:"inherit",boxShadow:`0 2px 8px ${A}44`}}>
            Conectar ao RHiD
          </button>
        </div>
      ):(
        <div style={{background:BR,borderRadius:12,padding:"16px",border:`1px solid ${BD}`,boxShadow:SD}}>
          <div style={{fontWeight:600,fontSize:14,color:TX,marginBottom:12}}>Conexão ativa</div>
          <div style={{fontSize:13,color:TX2,marginBottom:6}}>Conta: <strong style={{color:TX}}>{rhidCfg?.email}</strong></div>
          <div style={{fontSize:13,color:TX2,marginBottom:16}}>Sincronização: <strong style={{color:VE}}>automática a cada 30 segundos</strong></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <button onClick={onSincNow} style={{padding:"12px",borderRadius:8,border:`1px solid ${A}`,background:"#F0F7FA",color:A,fontWeight:600,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
              ↻ Sincronizar agora
            </button>
            <button onClick={onDesconectar} style={{padding:"12px",borderRadius:8,border:`1px solid ${VM}`,background:"#FDF0F0",color:VM,fontWeight:600,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>
              Desconectar
            </button>
          </div>
        </div>
      ...
      <div style={{background:"#FFFBF0",borderRadius:10,padding:"12px 14px",marginTop:4,border:`1px solid #F0D060`}}>
        <div style={{fontSize:12,color:"#7A6020",lineHeight:1.6}}>
          ⚡ <strong>Importante:</strong> Para que a integração automática funcione, você precisa ter permissão de acesso à API no seu cadastro do RHiD.
        </div>
      </div>
    </>
  );
...
export default function App(){
  ...
  // ── Botão voltar Android — navega para cartão em vez de sair do app ──
  useEffect(()=>{
    const telaInicial="cartao";
    // Empurra estado inicial no histórico
    window.history.pushState({tela},"");
    const onPopState=(e)=>{
      // Impede saída do app — volta para a tela anterior ou para cartão
      const telaAnterior=e.state?.tela||telaInicial;
      setTela(telaAnterior==="cartao"?telaInicial:telaAnterior);
      // Reempurra estado para manter o histórico funcional
      window.history.pushState({tela:telaAnterior},"");
    };
    window.addEventListener("popstate",onPopState);
    return()=>window.removeEventListener("popstate",onPopState);
  },[]);

  // Atualiza o histórico do browser quando muda de tela
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    const buscarPerfil=async()=>{
      try{
        const res=await fetch(`${SERVIDOR_URL}/api/rhid/funcionario`,{
    ...
    const buscarHistorico=async()=>{
      try{
        ...
        const res=await fetch(`${SERVIDOR_URL}/api/rhid/ponto_diario?inicio=${ini}&fim=${fim}`,{
          headers:{"Authorization":`Bearer ${token}`,"X-Cid-Rhid":RHID_CID}
        });
        if(!res.ok) return;
    ...
    const buscarMarcacoes=async()=>{
      try{
        ...
        const res=await fetch(
          `${SERVIDOR_URL}/api/rhid/ponto_diario?inicio=${dataHoje}&fim=${dataHoje}`,
  ...
  const bater=()=>{
    ...
    // Primeiro verifica se existe turno aberto (e1 sem s2) em qualquer dia recente — noturno sai no dia seguinte
    const idxAberto=todosRegs.findIndex(r=>r.e1&&!r.s2);
    if(idxAberto!==-1){
      // Existe turno aberto — registra a próxima batida NESTE mesmo dia (dia da entrada)
      const dia={...todosRegs[idxAberto]};
      if(!dia.s1){dia.s1=h;}
      else if(!dia.e2){dia.e2=h;}
      else if(!dia.s2){dia.s2=h;}
      else{msg("Ponto do dia completo.","info");return;}
      const arr=[...(regs[funcSel]||[])]; arr[idxAberto]=dia;
      setRegs(p=>({...p,[funcSel]:arr}));
      msg(`✅ Ponto registrado: ${h}`);
    } else {
      // Nenhum turno aberto — nova entrada
  ...
  // ── Tela de Integração RHiD — usa componente externo para não perder foco ──
  const TelaRhid=()=><TelaRhidComp
    rhidConectado={rhidConectado} rhidSincMens={rhidSincMens} rhidCfg={rhidCfg}
    rhidEmail={rhidEmail} setRhidEmail={setRhidEmail}
    rhidSenha={rhidSenha} setRhidSenha={setRhidSenha}
    onLogin={()=>{ if(!rhidEmail||!rhidSenha){msg("Preencha e-mail e senha.","err");return;} loginRhid(rhidEmail,rhidSenha); }}
    onDesconectar={desconectarRhid}
    onSincNow={()=>{setRhidSincMens("Sincronizando agora...");msg("Sincronizando com o RHiD...");}}
  />;
      ...
      {menu&&(
        <div style={{position:"fixed",inset:0,zIndex:500}} onClick={()=>setMenu(false)}>
          <div style={{position:"absolute",left:0,top:0,bottom:0,width:"82%",maxWidth:320,background:BR,boxShadow:"4px 0 20px rgba(0,0,0,0.15)",display:"flex",flexDirection:"column",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
            ...
            {/* Trocar perfil — apenas modo Funcionário visível */}
            {isAdmin&&(
            <div style={{padding:"12px 14px",borderTop:`2px solid ${BD}`,background:"#FAFAFA"}}>
              <div onClick={()=>{setPerfilAtivo("funcionario");msg("Perfil: Funcionário");}}
                style={{padding:"8px 10px",borderRadius:6,marginBottom:6,cursor:"pointer",
                  background:perfilAtivo==="funcionario"?"#E3F7FB":"#F5F5F5",
                  border:perfilAtivo==="funcionario"?`1.5px solid ${A}`:`1px solid ${BD}`}}>
                <div style={{fontWeight:700,fontSize:13,color:perfilAtivo==="funcionario"?A:TX}}>👤 Funcionário</div>
                <div style={{fontSize:11,color:TX2}}>Somente visualização</div>
              </div>
            </div>
            )}
            {/* Trocar funcionário */}
