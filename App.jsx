...
const MODALIDADES = {
  ...
  "12x36-noturno": { label:"12x36 Noturno",   jornada:660, intervalo:60, ep:"17:00", sp:"05:00", noturno:true,  legal:"Art. 59-A CLT", p1:"17:00 - 05:00", p2:"" },
...
function TelaLogin({onLogin}){
  ...
  const handleLogin=async()=>{
    ...
    if(modoLogin==="rhid"){
      ...
      }catch(e){setErro("Erro ao conectar. Verifique sua internet.");setCarregando(false);}
      return;
    }
...
export default function App(){
  ...
  const diasPeriodo=useMemo(()=>{
    // Gera TODOS os dias do período, marcando folga nos sem registro
    const dias=[];
    let dt=new Date(periodoIni+"T12:00:00");
    const dtFim=new Date(periodoFim+"T12:00:00");
    while(dt<=dtFim){
      const d=dt.toISOString().slice(0,10);
      const reg=todosRegs.find(r=>r.data===d);
      if(reg) dias.push(reg);
      else dias.push({data:d,folga:true});
