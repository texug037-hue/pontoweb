...
// Converte HH:MM ajustando para turnos noturnos que cruzam meia-noite
function toMinNoturno(t, refEntrada){
  if(!t) return null;
  const [h,m]=t.split(":").map(Number);
  let min=h*60+m;
  if(refEntrada!=null && min < refEntrada) min+=1440;
  return min;
}
...
function calcSaldo(e1,s1,e2,s2,sp,modalidade){
  ...
  const spMin=toMinNoturno(sp||m.sp,entMin);
  ...
  const jantaBonus=Math.max(0,60-jantaReal);
  const epMin=toMinNoturno(m.ep,null);
  const entradaBonus=Math.max(0,epMin-entMin);
  ...
  const handleLogin=async()=>{
    ...
    if(modoLogin==="rhid"){
      try{
        ...
        onLogin({id:1,nome:data.nome||email,email,
          ...
          <button onClick={()=>{
              ...
              if(modoLogin==="rhid"){
                fetch(`${SERVIDOR_URL}/api/rhid/login.svc`,{
                  ...
                  const tkn=data.accessToken||data.token||data.access_token;
                  if(!tkn){setErro("E-mail ou senha do RHiD incorretos.");setCarregando(false);return;}
                  onLogin({id:1,nome:data.nome||emailVal,email:emailVal,
                    empresa:"ILUMI INDUSTRIA E COMERCIO LTDA",cargo:"AUXILIAR ADMINISTRATIVO",
                    modalidade:"12x36-noturno",perfil:"funcionario",rhidToken:tkn});
...
export default function App(){
  ...
  const bater=()=>{
    ...
    // Verifica se existe turno aberto (entrada sem saída final) — noturno sai no dia seguinte
